    ...
    <div style={{ animation:"fadeUp 0.5s ease both" }}>
      ...
      {showForm && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.92)", zIndex:3000, overflowY:"auto", display:"flex", justifyContent:"center", padding:"24px 16px", backdropFilter:"blur(16px)" }}>
          <div style={{ background:"linear-gradient(135deg,#0d1629,#080f1c)", border:"1px solid rgba(56,189,248,0.2)", borderRadius:20, padding:"clamp(18px,4vw,32px)", width:"100%", maxWidth:600, height:"fit-content", alignSelf:"flex-start" }}>
                ...
                {CATS.map(cat => {
                  ...
                  return (
                    <button key={cat} onClick={() => selectCat(cat)}
                      style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:5, padding:"10px 6px", borderRadius:12, cursor:"pointer", border: active ? "2px solid "+cfg.border : "1px solid rgba(255,255,255,0.09)", background: active ? cfg.bg : "rgba(255,255,255,0.03)", outline:"none", transition:"all 0.15s" }}
                      onMouseEnter={e => { if (!active) e.currentTarget.style.background="rgba(255,255,255,0.07)"; }}
                      onMouseLeave={e => { if (!active) e.currentTarget.style.background="rgba(255,255,255,0.03)"; }}>
                      <span style={{ fontSize:"1.4rem", lineHeight:1 }}>{cfg.icon}</span>
...
fs.writeFileSync("src/pages/admin/AdminBlog.jsx", CONTENT, "utf8");
console.log("✅ AdminBlog.jsx completely replaced!");
